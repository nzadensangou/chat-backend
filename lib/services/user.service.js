...
export class UserService {
  ...
  static async register(payload) {
    ...
    // ✅ Avatar par défaut : on attribue à chaque nouvel utilisateur l'une
    // des 5 illustrations packagées dans l'app (assets/avatars/avatar_1.png
    // à avatar_5.png), au hasard. C'est stocké comme une vraie photo de
    // profil (colonne avatar_url) — pas recalculé à la volée côté client
    // comme avant — donc stable et partagé entre tous les appareils/clients.
    // Le préfixe "assets/" permet au frontend de savoir qu'il doit charger
    // ce fichier localement (Image.asset) plutôt que de le traiter comme
    // une URL réseau (Image.network) — voir UserAvatar.
    const defaultAvatarIndex = Math.floor(Math.random() * 5) + 1;
    const defaultAvatarUrl = `assets/avatars/avatar_${defaultAvatarIndex}.png`;

    // Create user in database
    const query = `
      INSERT INTO users (alanyaID, alanyaPhone, nom, pseudo, idPays, password, avatar_url, type_compte, is_online, last_seen, exclus, in_call, biometric, created_at)
  ...
  static async searchUsers(query, limit = 50, offset = 0) {
    // ✅ "Commence par" plutôt que "contient" : query% au lieu de %query%.
    // Taper "N" ne remonte plus "Jean" ou "Adriana" (lettre en milieu de
    // mot), seulement les noms/pseudos/numéros qui débutent par "N"/"n"
    // (la casse reste ignorée, via la collation par défaut de la table).
