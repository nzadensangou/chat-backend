...
class FCMService {
  ...
  async sendCallNotification(fcmToken, { callerId, callerName, callType }) {
    try {
      ...
      // ✅ Si Firebase nous dit que le token est invalide/expiré, on le
      // purge en base pour éviter de re-tenter (et re-logger) à chaque
      // appel ou message tant que le client n'a pas renvoyé un token valide.
      await this._cleanupTokenIfInvalid(error, fcmToken);
  ...
  /**
   * Detect Firebase errors that mean a token is permanently invalid,
   * and clear it from the users table so we stop retrying with it.
   * @private
   */
  async _cleanupTokenIfInvalid(error, fcmToken) {
    ...
    const isInvalidTokenError =
      INVALID_TOKEN_ERROR_CODES.includes(error.code) ||
      /not a valid FCM registration token/i.test(error.message || '');

    if (!isInvalidTokenError) return;

    try {
      // Import dynamique pour éviter une dépendance circulaire au chargement du module
      const { default: db } = await import('../db/index.js');
      await db.query('UPDATE users SET fcm_token = NULL WHERE fcm_token = ?', [fcmToken]);
      logger.info(
        { fcmToken: fcmToken.substring(0, 20) + '...' },
        'Invalid FCM token cleared from database'
      );
    } catch (cleanupError) {
      logger.warn({ error: cleanupError.message }, 'Failed to clean up invalid FCM token');
    }
  }
