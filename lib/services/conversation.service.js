...
export class ConversationService {
  ...
  static async _getEnrichedConversation(conversID, userId) {
    const query = `
      SELECT c.conversID, c.participantID, c.isGroup, c.lastMessage, c.lastMessageAt, c.isPinned, c.isArchived, c.unreadCount,
  ...
  static async markAsRead(conversID, userId) {
    // 1. Récupérer les messages non lus reçus par cet utilisateur
    //    (ceux qu'il n'a PAS envoyés lui-même)
    const unread = await db.getAll(
      `SELECT msgID, senderID FROM message
       WHERE conversationID = ? AND senderID != ? AND readAt IS NULL AND isDeleted = 0`,
      [conversID, userId]
    );

    if (unread.length > 0) {
      // 2. Marquer ces messages comme lus (colonne readAt déjà présente en BD)
      await db.query(
      ...
      // 3. Prévenir en temps réel chaque expéditeur concerné
      if (socketManager.isAvailable()) {
        const io = socketManager.getIO();
        const distinctSenders = [...new Set(unread.map((m) => m.senderID))];
        distinctSenders.forEach((senderId) => {
          io.to(`user:${senderId}`).emit('message:marked-read', {
    ...
    // 4. Comportement existant conservé : remettre le compteur à 0
    const query = `
    ...
    return { message: 'Marked as read', updatedCount: unread.length };
  }
