  ...
  static async createMessage(senderID, conversationID, payload) {
    ...
    try {
      if (socketManager.isAvailable()) {
        // 🔍 DEBUG TEMPORAIRE : combien de sockets sont réellement dans
        // chaque room au moment de l'émission ? Permet de savoir si le
        // destinataire a bien rejoint les rooms avant l'envoi.
        ...
        logger.info(
          {
            conversationID,
            recipientId,
            socketsInConversationRoom: convRoom ? convRoom.size : 0,
            socketsInUserRoom: userRoom ? userRoom.size : 0,
          },
          '🔍 DEBUG room membership avant émission message:new'
        );
