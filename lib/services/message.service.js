...
import { MESSAGE_TYPE_LABELS, MESSAGE_TYPE_CODES } from '../constants.js';
...
export class MessageService {
  ...
  static async createMessage(senderID, conversationID, payload) {
    ...
    // Le type stocké/retourné par atomicCreateMessage est le CODE ENTIER
    // (colonne INT en base). Tout ce qui part vers l'extérieur (socket,
    // FCM, réponse REST -> Flutter) doit utiliser le libellé texte.
    const typeLabel = MESSAGE_TYPE_LABELS[type] || 'text';
      ...
      if (fcmService.isReady()) {  // ← vérifie avant d'appeler
        ...
        if (recipient?.fcm_token && !recipient.is_online) {
          ...
          await fcmService.sendMessageNotification(recipient.fcm_token, {
            ...
            messagePreview: typeLabel === 'text' ? content.slice(0, 100) : `Vous a envoyé un(e) ${typeLabel}`,
            conversationId: conversationID,
          });
        }
      }
    ...
    return { ...createdMessage, type: typeLabel };
  }
  ...
  static async getMessages(conversationID, limit = 50, offset = 0) {
    ...
    const rows = await db.getAll(query, [conversationID, limit, offset]);
    // m.* renvoie le code entier brut (colonne INT) : on le reconvertit
    // en libellé texte pour que le frontend (qui teste type == 'audio'
    // etc.) reçoive le même format que pour un message qui vient d'être
    // envoyé — voir createMessage().
    return rows.map((row) => ({
      ...row,
      type: MESSAGE_TYPE_LABELS[row.type] || row.type,
    }));
  }
  ...
  static async searchMessages(conversationID, searchTerm, limit = 50, offset = 0) {
    ...
    const rows = await db.getAll(query, [conversationID, term, limit, offset]);
    return rows.map((row) => ({
      ...row,
      type: MESSAGE_TYPE_LABELS[row.type] || row.type,
    }));
  ...
  static async filterMessages(conversationID, type, limit = 50, offset = 0) {
    // `type` arrive en string ('audio', 'image'...) mais la colonne est
    // un INT : il faut convertir avant de filtrer, sinon la comparaison
    // `m.type = ?` ne matche jamais rien.
    const typeCode = MESSAGE_TYPE_CODES[type] || type;
    const query = `
      SELECT m.*, u.nom, u.pseudo, u.avatar_url
      FROM message m
      INNER JOIN users u ON m.senderID = u.alanyaID
    ...
    const rows = await db.getAll(query, [conversationID, typeCode, limit, offset]);
    return rows.map((row) => ({
      ...row,
      type: MESSAGE_TYPE_LABELS[row.type] || row.type,
    }));
