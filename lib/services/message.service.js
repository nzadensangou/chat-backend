  ...
  static async createMessage(senderID, conversationID, payload) {
    ...
    try {
      if (socketManager.isAvailable()) {
        socketManager.getIO().to(`conversation:${conversationID}`).emit('message:new', {
          conversationID,
          senderId: senderID,
          content,
          type,
        ...
        logger.debug(
          { conversationID, messageId: createdMessage.msgID },
          'Emitted message:new to conversation room'
        );
      }
    } catch (socketError) {
    ...
    try {
      if (fcmService.isReady()) {  // ← vérifie avant d'appeler
        const recipientId = await ConversationService.getOtherParticipant(conversationID, senderID);
