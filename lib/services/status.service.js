...
import { withTransaction, executeInsert, getNextId } from '../db/transaction-helper.js';

const TABLE = 'statut';

// ⚠️ (2026-07-13) La colonne `ID` de `statut` n'a ni AUTO_INCREMENT ni valeur
// par défaut (contrainte du schéma existant, qu'on ne peut pas modifier).
// Chaque INSERT sans ID explicite échoue avec "Field 'ID' doesn't have a
// default value". On applique donc le même pattern déjà utilisé ailleurs
// dans le projet pour ce genre de colonnes historiques (voir
// atomicCreateMessage / atomicCreateMeeting dans transaction-helper.js) :
// calculer l'ID nous-mêmes via MAX(ID)+1, dans une transaction pour éviter
// que deux créations concurrentes ne calculent le même ID.
...
export class StatusService {
  static async createStatus(userId, payload) {
    ...
    // ID généré manuellement (pas d'AUTO_INCREMENT sur cette table) et
    // insert exécutés dans la même transaction pour éviter qu'une création
    // concurrente ne réutilise le même ID entre le calcul et l'insertion.
    const statusId = await withTransaction(async (connection) => {
      const nextId = await getNextId(connection, TABLE, 'ID');

      const query = `
        INSERT INTO ${TABLE} (ID, alanyaID, type, text, mediaUrl, backgroundColor, createdAt, expiredAt)
        VALUES (?, ?, ?, ?, ?, ?, NOW(), DATE_ADD(NOW(), INTERVAL 24 HOUR))
      `;

      await executeInsert(connection, query, [
        nextId,
        userId,
        typeToCode(type),
        textValue,
