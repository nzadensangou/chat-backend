...
export async function atomicCreateMeeting(connection, meetingData) {
  const {
    ...
    // les colonnes déjà existantes avec leur sens réel :
    //   - start_time = l'heure PRÉVUE de début si fournie (sinon NOW(),
    //     comportement identique à avant pour un appel instantané)
    //   - duree      = la durée PRÉVUE en secondes si fournie (sinon 0,
    //     comme avant). endCall() l'écrasera avec la durée RÉELLE une
    //     fois l'appel terminé — exactement la philosophie de ce schéma :
    //     pas de date de fin figée, juste une durée.
  ...
  const meetingParams = plannedStartTime
    ? [meetingId, organizerId, plannedStartTime, typeCode, room, description, plannedDurationSeconds]
    : [meetingId, organizerId, typeCode, room, description, plannedDurationSeconds];
