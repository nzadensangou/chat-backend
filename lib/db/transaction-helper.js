...
export async function atomicCreateMessage(connection, messageData) {
  ...
  // ✅ FIX : on enregistre aussi `lastMessageType`, pas seulement le
  // contenu brut. Pour un média (image/audio/vidéo), `content` est l'URL
  // Cloudinary — sans le type qui l'accompagne, impossible pour le
  // frontend d'afficher un aperçu correct ("📷 Photo" au lieu de l'URL)
  // dans la liste des conversations.
  const updateQuery = `
    UPDATE conversation 
    SET lastMessage = ?, lastMessageType = ?, lastMessageAt = NOW() 
    WHERE conversID = ?
  `;
  await executeUpdate(connection, updateQuery, [content, type, conversationId]);
