...
export class MessageValidator extends BaseValidator {
   ...
   * @returns {number} Integer code for message type (the messages.type
   *   column is INT in the real database — see the hardcoded `1` comment
   *   in db/transaction-helper.js's atomicCreateConversation for
   *   confirmation. Do NOT change this to return a string: it will break
   *   the INSERT with a 500 error. The human-readable string label is
   *   reconstructed downstream, in MessageService.createMessage, via
   *   MESSAGE_TYPE_LABELS — that's where it belongs, not here.)
  ...
  static validateCreation(payload) {
    const { content, type = MESSAGE_TYPES.TEXT, mediaUrl } = payload;
