...
export class BaseValidator {
   ...
   * ✅ FIX: on attache statusCode = 400 sur l'erreur. Sans ça, handleError()
   * (backend/lib/utils/api-helpers.js) ne reconnaît le type d'erreur qu'en
   * cherchant des mots-clés ("validation", "not found", "already exists"...)
   * dans le texte du message — or la plupart des messages de validateur
   * (ex: "Username must contain only letters...") ne contiennent aucun de
   * ces mots-clés, et retombaient donc dans le cas par défaut : 500
   * Internal Server Error, alors qu'il s'agit d'une simple erreur de saisie
   * (400). handleError() vérifie déjà error.statusCode en PREMIER, donc ce
   * seul changement corrige TOUS les validateurs de l'app, pas seulement
   * l'inscription.
   * @param {string} message - Error message
   * @throws {Error} Validation error (avec statusCode 400)
   */
