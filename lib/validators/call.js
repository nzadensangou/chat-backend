...
export class CallValidator extends BaseValidator {
  ...
  static validateInitiation(payload) {
    const { receiverId, callType, room, title, scheduledStartTime, scheduledEndTime } = payload;

    this.validateRequiredFields(payload, ['receiverId', 'callType', 'room']);

    // ✅ FIX (bug "titre toujours 'video call'") : titre optionnel fourni par
    // l'utilisateur depuis MeetingCreateScreen. On ne le valide (trim +
    // longueur) que s'il est présent — sinon call.service.js retombe sur
    // son intitulé générique par défaut ("audio/video call").
    const validatedTitle =
      typeof title === 'string' && title.trim().length > 0
        ? this.validateLength(title, 1, 255, 'title')
        : null;
    ...
    // ✅ FIX (bug "réunion jamais dans À venir/En cours") : `scheduledEndTime`
    // ne sert plus qu'à la vérification de cohérence ci-dessus. `duree`
    // n'est PAS une durée prévue — c'est le temps RÉELLEMENT écoulé, rempli
    // uniquement par endCall(). On ne calcule donc plus de
    // "plannedDurationSeconds" à stocker dans `duree` : ça ne représenterait
    // rien de valide dans ce schéma, et c'était la cause du bug (une
    // réunion planifiée se retrouvait avec `duree` non nul dès sa création,
    // ce qui la faisait passer directement pour "terminée").
