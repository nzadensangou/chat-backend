...
export async function enforceStatusOwnership(userId, statusId) {
  ...
  const status = await db.getOne(`
    SELECT statusId, alanyaID FROM status WHERE statusId = ?
    LIMIT 1
  `, [statusId]);
...
export async function enforceStatusAccess(userId, statusId) {
  ...
  const hasAccess = await db.getOne(`
    SELECT s.statusId
    FROM status s
    WHERE s.statusId = ? AND (
      s.alanyaID = ? OR (
        s.visibility != 'private' AND
        EXISTS (SELECT 1 FROM preferredContact pc WHERE pc.alanyaID = ? AND pc.idFriend = s.alanyaID) AND
        NOT EXISTS (SELECT 1 FROM statusHiddenFrom shf WHERE shf.statusId = s.statusId AND shf.hiddenFromId = ?)
      )
    )
    LIMIT 1
  `, [statusId, userId, userId, userId]);
